...
public class UpdaterController {
    ...
    public List<Person> list() {
        ...
        return collection.find().skip(10).limit(10).map(doc -> {
            Person p = schemaVersionHandlerService.handleVersionChange(doc);
            mongoTemplate.getDb().getCollection("person", Person.class)
                    .replaceOne(Filters.eq("_id", doc.getString("_id")), p);
            return p;
        }).into(new ArrayList<>());
    }
    ...
    public @ResponseBody Person read(@PathVariable String id) {
        ...
        Person p = schemaVersionHandlerService.handleVersionChange(doc);
        mongoTemplate.getDb().getCollection("person", Person.class).replaceOne(Filters.eq("_id", doc.getString("_id")),
                p);
        return p;
    }
    ...
    @GetMapping("/person/batch-update")
    public String batchUpdate() throws InterruptedException {
        MongoCollection<BsonDocument> collection = mongoTemplate.getDb().getCollection("person", BsonDocument.class);
        int threads = 3;
        ...
        while (cursor.hasNext()) {
            ...
            if (subList.size() == batchSize) {
                ends.add(schemaVersionHandlerService.batchConvert(subList));
